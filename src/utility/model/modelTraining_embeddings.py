...
class CNN1D(nn.Module):
    ...
    def predict(self, X):
        ...
        Args:
            X: Input data (can be DataFrame, numpy array, or tensor)
...
class SimplerCNN1D(nn.Module):
    """A simplified 1D CNN for text classification based on embeddings."""
    
    MODEL_PATH = "../models/"
    
    def __init__(self, embedding_input, num_classes=None, label_column='user', 
                 embedding_prefix=None, dropout_rate=0.5, output_name=None, retrain=False):
        """
        Initialize the simplified CNN model.
        ...
        Args:
            ...
            num_classes: Number of classes for classification. If embedding_input is a DataFrame,
                         this will be inferred automatically.
            label_column: Name of the column containing the labels in the DataFrame.
            embedding_prefix: Prefix of columns containing embeddings in the DataFrame.
            dropout_rate: Dropout rate for regularization.
            output_name: Name to use when saving the model.
            retrain: Whether to overwrite an existing model with the same name.
        """
        super(SimplerCNN1D, self).__init__()

        # Setup model name and check for duplicates
        self.output_name = self.check_output_model_name(output_name)
        self.check_duplicate_model_name(self.output_name, retrain)
    ...
    def prepare_data(self, test_size=0.2, val_size=0.25, batch_size=32, random_state=42):
        ...
        # Create weighted sampler for balancing classes
        sampler = WeightedRandomSampler(
            weights=sample_weights,
            num_samples=len(train_dataset),
            replacement=True
    ...
    def augment_embeddings(self, embeddings, noise_level=0.05, training=True):
        """
        Augment embeddings with Gaussian noise during training.
        
        Args:
            embeddings: Input embeddings tensor
            noise_level: Standard deviation of the Gaussian noise
            training: Only apply augmentation during training
    ...
    def train_and_evaluate(self, test_size=0.2, val_size=0.2, batch_size=32, 
                     num_epochs=25, learning_rate=0.001, weight_decay=1e-4,
                     patience=7, noise_level=0.05, random_state=42, 
                     plot_results=True):
        """
        Complete pipeline that runs data preparation, model training, and evaluation.
        ...
        Args:
            ...
            noise_level: Level of noise for embedding augmentation
            random_state: Random seed for reproducibility
            plot_results: Whether to plot training history and confusion matrix
        ...
        # Using Adam optimizer with weight decay for L2 regularization
        optimizer = optim.Adam(self.parameters(), lr=learning_rate, weight_decay=weight_decay)
        
        # Learning rate scheduler - OneCycleLR often works well for CNNs
        ...
        for epoch in range(num_epochs):
            ...
            for embeddings, labels in train_bar:
                ...
                embeddings = self.augment_embeddings(embeddings, noise_level=noise_level, training=True)
            ...
            # Print epoch results
            print(f'Epoch {epoch+1}/{num_epochs}:')
            print(f'  Train Loss: {epoch_train_loss:.4f}, Train Acc: {epoch_train_acc:.4f}, Train F1: {epoch_train_f1:.4f}')
        ...
        report_df, cm, all_preds, all_labels = self.evaluate(test_loader)
    ...
    def evaluate(self, test_loader, class_names=None):
        ...
        Returns:
            report_df: DataFrame with classification metrics
            cm: Confusion matrix
    ...
    def plot_training_results(self, history, confusion_matrix=None):
        ...
        Args:
            ...
            confusion_matrix: Confusion matrix to plot (optional)
        """
        # Plot training history - loss, accuracy and F1 score
        ...
        plt.subplot(1, 3, 3)
        plt.plot(history['train_f1s'], label='Train F1')
    ...
    def check_duplicate_model_name(self, name, retrain):
        """Check if the model name already exists."""
        if os.path.exists(os.path.join(self.MODEL_PATH, name)) and not retrain:
            raise ValueError(f"Model with name {name} already exists. Allow retraining to overwrite.")
